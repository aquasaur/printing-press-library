...
func writeThroughCache(resourceType string, data json.RawMessage) {
	...
	if json.Unmarshal(data, &items) != nil || len(items) == 0 {
		...
		if json.Unmarshal(data, &envelope) == nil {
			for _, key := range []string{"results", "data", "items", "response", "games", "players", "friends"} {
			...
			if items == nil {
				...
				if idRaw, ok := envelope["steamid"]; ok {
					_ = db.Upsert(resourceType, strings.Trim(string(idRaw), "\""), data)
					return
				}
			}
	...
	for _, item := range items {
		...
		if id == "" {
			if idRaw, ok := obj["steamid"]; ok {
				id = strings.Trim(string(idRaw), "\"")
			}
		}
		if id == "" {
			if idRaw, ok := obj["appid"]; ok {
				id = strings.Trim(string(idRaw), "\"")
			}
		}
		if id != "" {
