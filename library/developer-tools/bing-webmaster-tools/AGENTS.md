...
This directory is a generated `bing-webmaster-tools-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
bing-webmaster-tools-pp-cli doctor --json
bing-webmaster-tools-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
bing-webmaster-tools-pp-cli which "<capability>" --json
bing-webmaster-tools-pp-cli <command> --help
```
...
```bash
bing-webmaster-tools-pp-cli <command> --agent
```
...
```bash
bing-webmaster-tools-pp-cli <command> --help
bing-webmaster-tools-pp-cli <command> --dry-run --agent
```
