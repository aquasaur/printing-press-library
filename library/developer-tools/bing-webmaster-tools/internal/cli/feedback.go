...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".bing-webmaster-tools-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BING_WEBMASTER_TOOLS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BING_WEBMASTER_TOOLS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bing-webmaster-tools-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.bing-webmaster-tools-pp-cli/feedback.jsonl.
When ` + "`BING_WEBMASTER_TOOLS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BING_WEBMASTER_TOOLS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "bing-webmaster-tools-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bing-webmaster-tools-pp-cli feedback list
  bing-webmaster-tools-pp-cli feedback list --limit 5
  bing-webmaster-tools-pp-cli feedback list --json`,
