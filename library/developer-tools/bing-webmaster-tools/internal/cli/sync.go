...
Exit codes & warnings:
  ...
  bing-webmaster-tools-pp-cli sync

  # Sync specific resources only
  bing-webmaster-tools-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  bing-webmaster-tools-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  bing-webmaster-tools-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  bing-webmaster-tools-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  bing-webmaster-tools-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/bing-webmaster-tools-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "block_urls":
		return db.UpsertBlockUrls(data)
	case "crawl":
		return db.UpsertCrawl(data)
	case "deep_links":
		return db.UpsertDeepLinks(data)
	case "feeds":
		return db.UpsertFeeds(data)
	case "fetch":
	...
	case "page_preview":
		return db.UpsertPagePreview(data)
	case "query_params":
		return db.UpsertQueryParams(data)
	case "sites":
