...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "bing-webmaster-tools-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "bing-webmaster-tools-pp-cli/0.1.0")
		}
...
//
// Hand-edit (not regenerated): the Bing Webmaster Tools API is a WCF .svc
// JSON endpoint, so every response is wrapped in a {"d": <payload>} envelope
// and dates are serialized in the legacy "/Date(ms)/" form. This function
// unwraps the envelope and normalizes those dates to ISO-8601 so downstream
// commands, --json output, and the local store see plain JSON. (Retro
// candidate: internal specs lack a `response_envelope_key` knob for this.)
func sanitizeJSONResponse(body []byte) []byte {
	...
	body = normalizeWCFDates(body)
	body = unwrapDEnvelope(body)
	return body
}

// unwrapDEnvelope returns the value of the single "d" key when body is a JSON
// object of the shape {"d": <payload>}, the envelope every Bing Webmaster
// Tools .svc method returns. Anything else (already-unwrapped JSON, an error
// object, a bare array, a non-object) is returned unchanged.
func unwrapDEnvelope(body []byte) []byte {
...
// wcfDateRe matches the legacy WCF JSON date literal "/Date(1714521600000)/"
// or "/Date(1714521600000+0000)/" as it appears (with escaped slashes) inside
// a JSON string value.
var wcfDateRe = regexp.MustCompile(`\\/Date\((-?\d+)(?:[+-]\d{4})?\)\\/`)

// normalizeWCFDates rewrites "/Date(ms)/" string values to RFC 3339 UTC so
// that consumers don't have to special-case Microsoft's legacy date format.
func normalizeWCFDates(body []byte) []byte {
	if !bytes.Contains(body, []byte(`\/Date(`)) {
		return body
	}
	return wcfDateRe.ReplaceAllFunc(body, func(m []byte) []byte {
		sub := wcfDateRe.FindSubmatch(m)
