...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("BING_WEBMASTER_TOOLS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "bing-webmaster-tools-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("BING_WEBMASTER_TOOLS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
