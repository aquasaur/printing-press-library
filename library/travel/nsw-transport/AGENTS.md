# Transport for NSW Open Data Printed CLI Agent Guide

This directory is a generated `nsw-transport-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
nsw-transport-pp-cli doctor --json
nsw-transport-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
nsw-transport-pp-cli which "<capability>" --json
nsw-transport-pp-cli <command> --help
```
...
```bash
nsw-transport-pp-cli <command> --agent
```
...
```bash
nsw-transport-pp-cli <command> --help
nsw-transport-pp-cli <command> --dry-run --agent
```
