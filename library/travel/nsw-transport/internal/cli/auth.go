...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Nsw Transport",
	}
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  nsw-transport-pp-cli auth setup\n  nsw-transport-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://opendata.transport.nsw.gov.au/data/user/register")
			fmt.Fprintln(w, "  Register an application on the Open Data Hub, then subscribe to the GTFS-Realtime, Trip Planner, and Live Traffic API products")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export NSW_OPENDATA_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  nsw-transport-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://opendata.transport.nsw.gov.au/data/user/register"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nsw-transport-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export NSW_OPENDATA_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  nsw-transport-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nsw-transport-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nsw-transport-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("NSW_OPENDATA_API_KEY") != "" {
				envStillSet = "NSW_OPENDATA_API_KEY"
			}
