...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nsw-transport-pp-cli doctor
  nsw-transport-pp-cli doctor --json
  nsw-transport-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export NSW_OPENDATA_API_KEY=<your-key>"
					report["auth_key_url"] = "https://opendata.transport.nsw.gov.au/data/user/register"
					report["auth_instructions"] = "Register an application on the Open Data Hub, then subscribe to the GTFS-Realtime, Trip Planner, and Live Traffic API products"
				} else {
					authConfigured = true
			...
			if os.Getenv("NSW_OPENDATA_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "NSW_OPENDATA_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "NSW_OPENDATA_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						verifyPath := "/v1/carpark/full-list"
						...
						authHeaders["User-Agent"] = "nsw-transport-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'nsw-transport-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'nsw-transport-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'nsw-transport-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'nsw-transport-pp-cli sync' to refresh."
	}
	return report
}
