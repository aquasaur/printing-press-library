...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  nsw-transport-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  nsw-transport-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"cameras", "carpark", "trip"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  nsw-transport-pp-cli workflow status

  # Show status as JSON
  nsw-transport-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("nsw-transport-pp-cli")
			}
