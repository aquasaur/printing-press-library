...
Exit codes & warnings:
  ...
  nsw-transport-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  nsw-transport-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  nsw-transport-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  nsw-transport-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  nsw-transport-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("nsw-transport-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/nsw-transport-pp-cli/data.db)")
...
func defaultSyncResources() []string {
	// Only `carpark` (the /v1/carpark/full-list "get everything" endpoint)
	// produces a flat, enumerable array suitable for the local store. The
	// `cameras` resource returns a GeoJSON FeatureCollection (array nested
	// under `features`, ids nested under `properties`) and `trip` is a search
	// API with no enumerable collection — both are fetched live by their
	// commands instead. Use `refresh` to populate the fuel-price snapshot store.
	return []string{
		"carpark",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		// Absolute URLs because these resources override the spec base_url.
		"cameras": "https://api.transport.nsw.gov.au/v1/live/cameras",
		"carpark": "https://api.transport.nsw.gov.au/v1/carpark/full-list",
		"trip":    "https://api.transport.nsw.gov.au/v1/tp/stop_finder",
	}
...
var resourceIDFieldOverrides = map[string]string{
	// Park & Ride facilities key on tsn (Transit Stop Number); they have no
	// plain "id" field, so without this override every carpark item fails ID
	// extraction and the whole sync run reports failure.
	"carpark": "tsn",
}
