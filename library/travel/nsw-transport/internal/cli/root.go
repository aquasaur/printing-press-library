...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "nsw-transport-pp-cli",
		Short: `Nsw Transport CLI — Every NSW transport, traffic, and fuel feed in one CLI, plus cross-source commands no single API offers.`,
		Long: `Nsw Transport CLI — Every NSW transport, traffic, and fuel feed in one CLI, plus cross-source commands no single API offers.
...
Health check: run 'nsw-transport-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("nsw-transport-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'nsw-transport-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCarparkCmd(flags))
	...
	rootCmd.AddCommand(newCamerasPromotedCmd(flags))
	rootCmd.AddCommand(newHazardsPromotedCmd(flags))
	// Hand-built sources & cross-source commands (GTFS-Realtime protobuf,
	// FuelCheck OAuth, and the location-intelligence transcendence commands).
	rootCmd.AddCommand(newRealtimeCmd(flags))
	rootCmd.AddCommand(newFuelCmd(flags))
	rootCmd.AddCommand(newCommuteCmd(flags))
	rootCmd.AddCommand(newFuelStopCmd(flags))
	rootCmd.AddCommand(newWhereisCmd(flags))
	rootCmd.AddCommand(newStationCmd(flags))
	rootCmd.AddCommand(newCamerasNearCmd(flags))
	rootCmd.AddCommand(newDisruptionsCmd(flags))
	rootCmd.AddCommand(newFuelDriftCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("nsw-transport-pp-cli %s\n", version)
		},
	}
}
