...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".nsw-transport-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NSW_TRANSPORT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NSW_TRANSPORT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nsw-transport-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.nsw-transport-pp-cli/feedback.jsonl.
When ` + "`NSW_TRANSPORT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NSW_TRANSPORT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nsw-transport-pp-cli feedback list
  nsw-transport-pp-cli feedback list --limit 5
  nsw-transport-pp-cli feedback list --json`,
